...
object MostPopularSuperhero {
  ...
  def main (args :Array[String]){
    ...
    val names = sc.textFile("C:/Users/aman_PC/Downloads/SparkScala/SparkScala/marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-apperarance data
    val lines = sc.textFile("C:/Users/aman_PC/Downloads/SparkScala/SparkScala/marvel-graph.txt")
    
    // Convert to (heroID, number of connections) RDD
    val pairings = lines.map(countCoOccurences)
    
    pairings.collect()
    //pairings.foreachPartition(println)
